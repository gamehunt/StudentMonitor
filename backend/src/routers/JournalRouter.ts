...
async function parseTotalMarksQuery(req): Promise<TotalMarksReqData> {
    ...
    let lesson = await LESSON_PROVIDER.getLessonsById(parseInt(req.params.lesson))

    return {valid: true, start: start, end: end, group: group, lesson: lesson}
}
...
JOURNAL_ROUTER.route('/total/:group/:lesson')
    .get(async (req, res) => {
        ...
        data.marks        = await JOURNAL_PROVIDER.getAllMarks(reqd.start, reqd.end, reqd.group, reqd.lesson)
